...
def test_matvec():
    ...
    assert np.allclose(
        mkl_spblas.mkl_matvec(A, x), Ax_bench, atol=atol, rtol=rtol
    )
    # For CSC format.
    assert np.allclose(
...
def test_transpose_matvec():
    ...
    assert np.allclose(
        mkl_spblas.mkl_matvec(A, x, transpose=True),
        ATx_bench, atol=atol, rtol=rtol
    )
    # For CSC format.
    assert np.allclose(
...
def simulate_matrix(nonzero_frac=.05, shape=[1000, 500], seed=0):
    ...
    A = sp.sparse.csr_matrix(
        (np.random.randn(nnz), (row_index, col_index)), shape
    )
    return A
